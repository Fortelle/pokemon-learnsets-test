...
dest_folder = os.path.join(root_path, 'dest')
...
def create_move_object(method: str, entry: MoveEntry):
  ...
  match method:
    ...
    case 'levelup':
      ...
      move_object['level'] = int(entry.value)
    case 'tm' | 'tr':
      move_object['move'] = entry.index
      move_object['method'] = method
...
for game_key, filenames in index_data.items():
  ...
  for file_key in filenames:
    ...
    if not(method_key in method_keys):
  ...
  dest_learnset_collection = []
  for pokemon, moves in merged_learnset_collection.items():
    dest_learnset_collection.append({
    'pokemon': pokemon.number,
    'form': pokemon.form,
    'moves': moves,
    })

  dest_text = json.dumps(dest_learnset_collection, indent=2)
  dest_filepath = os.path.join(dest_folder, game_key + '.json')
...
print(f'Performed methods: {method_keys}.')
